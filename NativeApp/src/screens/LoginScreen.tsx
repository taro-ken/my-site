...
import { getIdentityAuthErrorMessage, signInWithEmailForApp } from '../lib/identityAuth';
import { useMembershipAuth } from '../context/MembershipAuthContext';
import type { MembershipStackParamList } from '../navigation/types';

type Props = NativeStackScreenProps<MembershipStackParamList, 'Login'>;

export default function LoginScreen({ navigation, route }: Props) {
  const insets = useSafeAreaInsets();
  const { authError, clearAuthError } = useMembershipAuth();
  ...
  const handleLogin = async () => {
    ...
    try {
      await signInWithEmailForApp(trimmedEmail, trimmedPassword);
    } catch (error) {
      setErrorMessage(getIdentityAuthErrorMessage(error));
    } finally {
  ...
  return (
    ...
    >
      <View style={styles.card}>
        ...
        <Text style={styles.subtitle}>ログインしてメンバーシップを管理。</Text>

        {route.params?.registered ? (
          <Text style={styles.successMessage}>登録が完了しました。ログインしてください。</Text>
        ) : null}
        ...
        <Pressable accessibilityRole="button" onPress={() => navigation.navigate('Register')} style={styles.linkButton}>
          <Text style={styles.linkText}>
            アカウントを持っていませんか？ <Text style={styles.linkTextStrong}>新規登録</Text>
          </Text>
        </Pressable>
      </View>
    </KeyboardAvoidingView>
